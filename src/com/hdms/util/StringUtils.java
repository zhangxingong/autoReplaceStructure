...
public class StringUtils {
	...
	public static String replaceTemplateStr(String str, String paternBegin,
			String paternEnd, Map map) throws IllegalAccessException,
			InvocationTargetException, NoSuchMethodException {
		int idx = str.indexOf(paternBegin);
		while (idx >= 0) {
			int lastIdx = str.indexOf(paternEnd, idx);
			if (lastIdx > idx) {
				String key = str.substring(idx + paternBegin.length(), lastIdx);
				String value = "";
				// if the key started with coboHDMS, it is just a string, not an
				// Object
				if (key != null && key.startsWith("coboHDMS.")) {
					value = map.get(key).toString();
				} else {
					Object o = PropertyUtils.getProperty(map, key);

					if (o != null)
						value = o.toString();
				}
				str = str.substring(0, idx) + value
						+ str.substring(lastIdx + paternEnd.length());
				idx = str.indexOf(paternBegin, idx);
	...
	public static String toJSValueString(String s) {
	...
	public static String html2smtl(String input,boolean replaceBR) {
	    return  html2smtl(input, replaceBR, false, false);
    }

    public static String html2smtl(String input, boolean replaceBR, boolean replaceAllBR, boolean outputHtml) {
        ...
        for (Iterator iter = imgs.iterator(); iter.hasNext();) {
            Element ele = (Element) iter.next();
            String image = ele.attr("src");
            String classAttr = ele.attr("class");
            if ("cobo_video".equals(classAttr)) {
                continue;
            }
            if (image != null && image.length() > 10 ) {
                TextNode a=null;
                if(image.indexOf("/emoticons/img/smiley-")==-1) {
                    a= new TextNode("[image]" + image + "[/image]", "");
                }else{
                    String m = image.substring(image.indexOf("/emoticons/img/smiley-")+"/emoticons/img/smiley-".length());
                    a= new TextNode("[smiley]" + m.substring(0,m.length()-4) + "[/smiley]", "");
                }
                ele.replaceWith(a);
            }
        }
        ...
        for(Iterator iterator = links.iterator();iterator.hasNext();){
            ...
            ele.text("");
            TextNode a = new TextNode("[link]"+text+"$$$"+src+"[/link]", "");
            ele.after(a);
        }
        String text = outputHtml ? doc.body().html() : doc.text();
        if (replaceBR) {
            text = text.replaceAll("br2nl", "\n").trim();
            if (replaceAllBR) {
	...
	public static String addHtmlClass(String src, String key, String class_name){

    	String result = src;
    	Document doc = Jsoup.parseBodyFragment(src);
	...
	public static String addHtmlId(String src, String key, String id_name){
	...
	public static String getHtmlId(String src, String key){
